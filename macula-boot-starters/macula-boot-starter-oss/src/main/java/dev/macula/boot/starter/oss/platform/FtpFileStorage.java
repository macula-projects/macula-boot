...
public class FtpFileStorage implements FileStorage {
    ...
    public Ftp getClient() {
        FtpConfig config =
            FtpConfig.create().setHost(host).setPort(port).setUser(user).setPassword(password).setCharset(charset)
                .setConnectionTimeout(connectionTimeout).setSoTimeout(soTimeout)
                .setServerLanguageCode(serverLanguageCode).setSystemKey(systemKey);
    ...
    public boolean save(FileInfo fileInfo, UploadPretreatment pre) {
        ...
        try (InputStream in = pre.getFileWrapper().getInputStream()) {
            ...
            if (thumbnailBytes != null) { //上传缩略图
                ...
                client.upload(getAbsolutePath(basePath + fileInfo.getPath()), fileInfo.getThFilename(),
