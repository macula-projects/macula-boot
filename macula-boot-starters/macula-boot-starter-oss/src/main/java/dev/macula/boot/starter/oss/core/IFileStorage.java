...
import dev.macula.boot.starter.oss.utils.ZipUtils;
...
public interface IFileStorage {
    ...
    default void download(HttpServletResponse response, List<String> objectNameList) throws Exception {
        if (ObjectUtil.isNotEmpty(objectNameList)) {
            if (objectNameList.size() == 1) {
                ...
                for (String objectName : objectNameList) {
                    ...
                    fileEntries.add(
                        ZipUtils.FileEntry.builder().inputStream(is).zipEntry(new ZipEntry(objectName)).build());
                }
                // 批量下载
