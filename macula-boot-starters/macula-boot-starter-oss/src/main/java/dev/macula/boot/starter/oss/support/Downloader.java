...
import dev.macula.boot.starter.oss.aspect.DownloadAspectChain;
import dev.macula.boot.starter.oss.aspect.DownloadThAspectChain;
    ...
    public void inputStream(Consumer<InputStream> consumer) {
        if (target == TARGET_FILE) {    //下载文件
            new DownloadAspectChain(aspectList,
                (_fileInfo, _fileStorage, _consumer) -> _fileStorage.download(_fileInfo, _consumer)).next(fileInfo,
                fileStorage, in -> consumer.accept(
                    progressListener == null ? in : new ProgressInputStream(in, progressListener, fileInfo.getSize())));
        } else if (target == TARGET_TH_FILE) {  //下载缩略图文件
            new DownloadThAspectChain(aspectList,
                (_fileInfo, _fileStorage, _consumer) -> _fileStorage.downloadTh(_fileInfo, _consumer)).next(fileInfo,
                fileStorage, in -> consumer.accept(progressListener == null ? in
