...
import dev.macula.boot.starter.oss.aspect.DeleteAspectChain;
import dev.macula.boot.starter.oss.aspect.ExistsAspectChain;
import dev.macula.boot.starter.oss.aspect.FileStorageAspect;
import dev.macula.boot.starter.oss.aspect.UploadAspectChain;
import dev.macula.boot.starter.oss.config.FileStorageProperties;
...
import dev.macula.boot.starter.oss.support.*;
...
public class FileStorageService implements DisposableBean {
    ...
    public boolean exists(FileInfo fileInfo) {
        if (fileInfo == null)
            return false;
        return new ExistsAspectChain(aspectList, (_fileInfo, _fileStorage) -> _fileStorage.exists(_fileInfo)).next(
            fileInfo, getFileStorageVerify(fileInfo));
    }
    ...
    public UploadPretreatment of(File file) {
        try {
            ...
            pre.setFileWrapper(new MultipartFileWrapper(
                new MockMultipartFile(file.getName(), file.getName(), contentType,
    ...
    public UploadPretreatment of(URL url) {
        try {
            ...
            pre.setFileWrapper(
                new MultipartFileWrapper(new MockMultipartFile(url.toString(), name, contentType, bytes)));
            return pre;
        } catch (Exception e) {
