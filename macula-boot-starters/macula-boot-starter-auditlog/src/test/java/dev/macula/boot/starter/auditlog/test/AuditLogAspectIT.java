...
import dev.macula.boot.starter.auditlog.test.listener.AuditLogTestListener;
...
/**
 * <b>AuditLogAspectIT</b> 审计日志切面集成测试类
 *
 * <p>测试 @AuditLog 注解是否正常工作，操作日志是否正确发布事件
 *
 * @author Rain
 * @since 2024/04/07
 */
...
public class AuditLogAspectIT {

    @Autowired
    private MockMvc mockMvc;

    @Autowired
    private AuditLogTestListener listener;
    ...
    void testAuditLogAspectInjected() {
        ...
        Assertions.assertNotNull(listener, "AuditLogTestListener should be injected");
    }
    ...
    void testCreateUserWithAuditLog() throws Exception {
        ...
        // when
        mockMvc.perform(post("/api/v1/test/user").contentType(MediaType.APPLICATION_JSON).content(userJson))
        ...
        Assertions.assertEquals(dev.macula.boot.starter.auditlog.enums.BusinessType.INSERT.ordinal(), event
            .getBusinessType(), "BusinessType.INSERT ordinal should be correct");
        ...
        Assertions
            .assertEquals("dev.macula.boot.starter.auditlog.test.controller.AuditLogTestController.createUser()", event
                .getMethod(), "Method signature should be correct");
    }
    ...
    void testUpdateUserWithAuditLog() throws Exception {
        ...
        // when
        mockMvc.perform(put("/api/v1/test/user/1").contentType(MediaType.APPLICATION_JSON).content(userJson))
            .andExpect(status().isOk());
        ...
        Assertions.assertEquals(dev.macula.boot.starter.auditlog.enums.BusinessType.UPDATE.ordinal(), event
            .getBusinessType()); // BusinessType.UPDATE
    ...
    void testDeleteUserWithAuditLog() throws Exception {
        // when
        mockMvc.perform(delete("/api/v1/test/user/1")).andExpect(status().isOk());

        // then
        Assertions.assertEquals(1, listener.getReceivedEvents().size());
        OperLogEvent event = listener.getReceivedEvents().get(0);
        Assertions.assertEquals("用户管理", event.getTitle());
        Assertions.assertEquals(dev.macula.boot.starter.auditlog.enums.BusinessType.DELETE.ordinal(), event
            .getBusinessType()); // BusinessType.DELETE
        Assertions.assertEquals(BusinessStatus.SUCCESS.ordinal(), event.getStatus());
    ...
    void testGetUserWithAuditLog() throws Exception {
        // when
        mockMvc.perform(get("/api/v1/test/user/1")).andExpect(status().isOk());

        // then
        Assertions.assertEquals(1, listener.getReceivedEvents().size());
        OperLogEvent event = listener.getReceivedEvents().get(0);
        Assertions.assertEquals("用户管理", event.getTitle());
        Assertions.assertEquals(dev.macula.boot.starter.auditlog.enums.BusinessType.SELECT.ordinal(), event
            .getBusinessType()); // BusinessType.SELECT
        Assertions.assertEquals(BusinessStatus.SUCCESS.ordinal(), event.getStatus());
