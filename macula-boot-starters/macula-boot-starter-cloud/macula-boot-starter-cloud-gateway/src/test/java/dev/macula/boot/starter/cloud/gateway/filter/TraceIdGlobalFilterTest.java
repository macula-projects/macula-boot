...
/**
 * Tests trace response handling when optional tracing infrastructure is absent.
 *
 * @author rain
 * @since 6.1.0
 */
class TraceIdGlobalFilterTest {
    ...
    @Test
    void shouldContinueWithoutBraveTracingBean() {
        try (AnnotationConfigApplicationContext context = new AnnotationConfigApplicationContext()) {
            ...
            assertThatCode(() -> filter.filter(exchange, ignored -> Mono.empty()).block()).doesNotThrowAnyException();
        }
    }
}
