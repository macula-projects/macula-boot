...
- `macula-boot-starters`：按能力拆分的 Spring Boot Starter；云能力位于其 `macula-boot-starter-cloud` Maven 聚合模块及子模块中，聚合模块当前不承载生产代码。
...
- 云厂商特有逻辑留在 Alibaba/Tencent 对应模块。新增跨厂商共享代码前先明确稳定 API 边界，不要默认放入当前仅用于聚合的 cloud 模块。
- 修改 archetype 时，在 `src/main/resources/archetype-resources` 中核对生成后的路径、占位符和父子 POM。
- 公共 API 变更应先搜索仓库内所有调用方，并考虑下游兼容性。
- 聚合 POM 或模块清单变化时，同步核对根 README 的项目结构和能力列表。
