...
## Deviations
- Corrected the existing dependency-management typo `macuala-boot-starter-async` to `macula-boot-starter-async`. This was required so the planned OperationLog async regression test can consume the Async Starter without declaring a child-module version; it does not change implementation order, risk, or proof scope.
- Added `macula-boot-examples/docker/docker-compose.yml` to the implementation file set so the existing Alibaba/Tencent application service anchors can receive the Collector endpoint and signal switches when the planned observability overlay is combined. Without this wiring the standalone backend overlay would start but no containerized example could export to it; existing profile membership, ports and default-disabled exporter behavior remain unchanged.
- Removed the specified `management.opentelemetry.enabled` condition and documented it as unsupported because Spring Boot 4.0.8's effective `OpenTelemetryProperties` contains only `resource-attributes`, and `OpenTelemetrySdkAutoConfiguration` has no property condition. The supported shutdown contract is to disable the Metrics, Traces and Logs exporters individually; `macula.observability.enabled` still controls only Macula's incremental configuration. This corrects an accepted-spec assumption using the resolved Boot API without adding a custom alias that would misleadingly claim to disable Boot's SDK.
- Added a minimal managed-async logging probe to the Alibaba consumer example and invoked it from the deterministic echo route. This supplies the accepted runtime correlation proof with distinct synchronous and managed-async log records under the same Trace ID; it does not alter the example API response contract.
- Added `io.github.openfeign:feign-micrometer` to `macula-boot-starter-feign` after the real Alibaba chain proved that Spring Cloud OpenFeign had no `MicrometerObservationCapability` on its runtime classpath. Without it the Consumer-to-Provider call started a separate trace; with it, the pinned end-to-end stack exported one W3C trace across Gateway, Consumer and Provider.
- Ordered `ObservabilityAutoConfiguration` after Spring Boot's `OpenTelemetrySdkAutoConfiguration`. The real container condition report proved the Logback installer was evaluated before Boot created its `OpenTelemetry` Bean, so OTLP logs were otherwise absent even though structured stdout contained correlation fields.
- Corrected observability overlay bind-mount paths for both standalone and combined Compose base-file resolution, ran local Loki/Tempo with root only to make named example volumes writable, and removed the Tempo 3.0.3-incompatible top-level `compactor` block. These are backend-example compatibility fixes discovered by validating the exact pinned images; application contracts and host ports are unchanged.
- After deploy review, initialize an existing user-declared official `MACULA_OTEL` appender with the managed `OpenTelemetry` instance without taking ownership, and add a regression test. The former name-only backoff left the official appender buffering and eventually dropping records.
- After deploy review, change ordinary Starter/example/Archetype sampling examples from `1.0` to `0.1`, preserve Docker acceptance at an explicit `1.0`, honor `OTEL_SERVICE_NAME` in generated projects, remove stale Prometheus endpoint exposure, and replace process-version health checks with HTTP readiness probe containers. These corrections align the implementation with the already accepted production/acceptance boundary and do not change the signal architecture.
